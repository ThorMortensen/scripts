...
SearchArmCompiler() {
  echo -e "\033[0;32mSearching for ARM compilers..\033[0m"
  gccpath=$(locate -b -q 'arm-linux-gnueabihf-gcc' | grep "04.00.00.04" | grep "arm-linux-gnueabihf-gcc" | sort -u | head -n 1)
  gxxpath=$(locate -b -q 'arm-linux-gnueabihf-g++' | grep "04.00.00.04" | grep "arm-linux-gnueabihf-g++" | sort -u | head -n 1)
...
SearchNativeCompiler() {
  echo -e "\033[0;32mSearching for X86/X86_64 compilers..\033[0m"
  gccsimpath=$(which gcc)
  gxxsimpath=$(which g++)
  if [ "$gccsimpath" == "" ] || [ "$gxxsimpath" == "" ]; then
    echo -e "\033[1;31mUnable to find compiler\033[0m\n"
  else
    echo -e "\033[1;36mC compiler set to $gccsimpath \nC++ compiler set to $gxxsimpath\033[0m\n"
  fi
}
...
main() {
  ...
  if [ "$err" -ne 0 ] || [ "$no_args" = 1 ]; then
    echo "Struers tool for compiling code for ARM & X86/X86_64"
    echo " "
    echo "Major options:"
  ...
  if [ "$Opt_B" = 1 ]; then
    ...
    CompileIndividually "struers-app" "ARM"
    exit 0

  elif [ "$Opt_Ci_X86" = 1 ]; then
    CompileIndividually "struers-app" "X86"
    exit 0
...
cat <<EOT > makefile
################################################################################
################### STRUERS AUTOMATICALLY GENERATED MAKEFILE ###################
################################################################################
